...
import { WrappedHashi721 } from "../typechain-types";
...
describe("Unit Test for WrappedHashi721", function () {
  ...
  it("initialize", async function () {
    ...
    await expect(wrappedHashi721.initialize()).to.revertedWith(
  ...
  it("mint", async function () {
    ...
    await expect(
      wrappedHashi721
        .connect(malicious)
        .mint(signer.address, tokenId_1, tokenURI)
  ...
  it("burn", async function () {
    ...
    await expect(
      wrappedHashi721.connect(malicious).burn(tokenId)
    ).to.revertedWith("Ownable: caller is not the owner");
  ...
  it("supportsInterface", async function () {
    const wrappedHashi721InterfaceId = "0x10430810";
    const erc721InterfaceId = "0x80ac58cd";
    expect(
      await wrappedHashi721.supportsInterface(wrappedHashi721InterfaceId)
    ).to.equal(true);
    expect(await wrappedHashi721.supportsInterface(erc721InterfaceId)).to.equal(
      true
    );
  });
});
